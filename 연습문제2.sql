...
INSERT INTO `bank_trasaction` VALUES
...
#실습2-8
SELECT `a_item_dist`,`c_no` FROM `bank_account`;

#실습2-9
SELECT  distinct `a_item_dist`,`a_item_name` FROM `bank_account`;
 
#실습2-10
SELECT * FROM `bank_account` WHERE  `a_item_dist`='S1'  AND
`a_balance` >= 1000000;
...
#실습2-12
SELECT * FROM `bank_transaction` WHERE `c_dist` = 2;
...
SELECT * FROM `bank_account`
WHERE `a_balance` = (SELECT MAX(`a_balance`) FROM
`bank_account` WHERE `a_item_dist` = 'S1');
...
#실습2-22
SELECT t_dist,COUNT(`t_no`) FROM `bank_transaction` GROUP BY
`t_dist`;


#실습2-23
SELECT 
`t_a_no`, `t_dist`, SUM(`t_amount`)
FROM `bank_transaction`
WHERE `t_dist` = 1
GROUP BY `t_a_no`
ORDER BY `t_a_no`;
