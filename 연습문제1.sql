...
ALTER TABLE `Order` CHANGE COLUMN `oderNo` `orderNo` INT;
ALTER TABLE `Order` CHANGE COLUMN `oderId` `orderId` VARCHAR(10);
SELECT * FROM `Order`;
#실습1-3
...
#실습1-14
SELECT * FROM `Product` 
WHERE `price` >=2000 AND `price` <=2000   ;
